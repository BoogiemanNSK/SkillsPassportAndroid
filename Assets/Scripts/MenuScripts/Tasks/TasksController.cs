...
using MenuScripts.Tasks.Case;
using UnityEngine;
using UnityEngine.UI;
using Random = System.Random;

namespace MenuScripts.Tasks
{
    public class TasksController : MonoBehaviour {
        ...
        public CaseController CaseControl;
        ...
        public async void OnEnter(int clickedSkill) {
            ...
            if (userQuery.Value.SkillCasePassed[clickedSkill])
                CaseBtn.interactable = false;
        ...
        public async void OnCaseClick()
        {
            // Create random case task for skill
            var randomTask = await GetRandomTask(2, _currentSkill);
            if (randomTask == null) {
                Debug.Log("No such tasks left!");
                return;
            }
            ...
            CaseBtn.interactable = false;
            CaseControl.gameObject.SetActive(true);
            CaseControl.OnEnter(randomTask.Value);
            _readyForUpdate = false;
            gameObject.SetActive(false);
        }
